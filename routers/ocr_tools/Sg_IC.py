...
#['Country/Place of birth', 'Date of birth', 'Name', 'REPUBLIC OF SINGAPORE', 'Race', 'Sex', 'IDENTITY CARD NO.']
...
def label_detect(datafolder):
    ...
    label_list2.remove('CHINESE')
    label_list2.remove('M')
...
def sg_IC(IC_file,label_list):
    ...
    for i in IC_doc:
        for n,j in enumerate(label_list):
            #if j in i[1][0] and ' ' in i[1][0] and len(j)+1 < len(i[1][0]):
    ...
    image = IC_file
    random_y=random.sample(range(int(image.shape[0]/2), image.shape[0]), 200)
    random_x=random.sample(range(int(image.shape[1]/2), image.shape[1]), 200)
    sum_blue=0
    sum_red=0
    for i in range(100):
        sum_blue = sum_blue + image[random_y[i],random_x[i]][0]
        sum_red = sum_red + image[random_y[i],random_x[i]][2]
    #print(sum_blue,sum_red)
    if sum_blue > sum_red:
        output['Type'] = 'Blue'
    else:
        output['Type'] = 'Pink'
...
if __name__ == "__main__":
    datafolder='/Users/feiranyang/Documents/Python_projects/Doc_OCR/Singapore_IC/Local'
    label_list = label_detect(datafolder)
    for file in os.listdir(datafolder+'/Cropped_image/'):
        if os.path.splitext(file)[1] in ['.png', '.jpg', '.jpeg']:
            sg_IC(datafolder+'/Cropped_image/'+file,label_list)
